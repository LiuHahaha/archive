...
struct
  ...
  fun toString (M cols) =
      Seq.toString (cols, fn s => "\n" ^ (Seq.toString (s, Int.toString)))

  type 'a matrix = 'a absmat

  fun size (M cols) = (Seq.length cols, Seq.length (Seq.nth cols 0))
                        handle Seq.Range => (0, 0)

  fun sub (M cols) (i, j) = Seq.nth (Seq.nth cols i) j

  fun repeat x (width, height) =
      M (Seq.repeat ((Seq.repeat (x, height)), width))

  fun tabulate f (h, w) =
      M (Seq.tabulate (fn i => Seq.tabulate (fn j => f (i,j)) h) w)

  fun map f (M cols) = M (Seq.map (fn c => Seq.map f c) cols)
  ...
  fun update (m as (M cols)) ((i, j), x) =
      ...
      in
        M (Seq.tabulate (update_one cols i
                           (Seq.tabulate
                              (update_one (Seq.nth cols i) j x) h)) w)
      end
  ...
  fun from_rows rows = 
      ...
      in
          M (Seq.tabulate (fn j => (Seq.tabulate (fn i => sub iftheywerecols (i, j)) w)) h)
      end
