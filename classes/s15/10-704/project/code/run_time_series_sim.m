...
Ts = num2cell([125 250 375 500]); % number of time points (samples)
data_types = {'simultaneous only', 'lagged only', 'both'};
est_types = {'KDE', 'KNN'}; % types of entropy estimators to use
test_types = {'corr'}; %{'MI', 'TE', 'MIR', 'corr'}; % dependence measure to use (CHANGED THIS LAST)
...
sigma_Ys = num2cell([0:9]); % variance of Gaussian noise on Y
...
save('results_main_corr', 'results');
