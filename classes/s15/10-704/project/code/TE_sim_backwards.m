...
T = 250; % number of time points (samples)
nperms = 1000; % number of permutations (for permutation test)
sigma = 1; % variance of shared Gaussian signal
sigma_Ys = 1:10; % variance of Gaussian noise on Y
lag = 1; % time lag (of Y behind X)
...
for sigma_Yi = 1:n_sigma_Ys
  ...
  parfor trial = 1:n_trials
    ...
    TE_p(trial) = TE_test(Ys, Xs, lag, nperms);
